...
from istream_player.modules.bw_meter.bandwidth_cont import BandwidthMeterCont
...
from istream_player.modules.mpd.hls_provider_impl import HLSProviderImpl
...
from istream_player.modules.scheduler.dash_scheduler import SchedulerImpl
from istream_player.modules.scheduler.mp4_scheduler import MP4SchedulerImpl
from istream_player.modules.scheduler.dash_scheduler_360 import Scheduler360

from istream_player.utils.async_utils import try_await
...
class PlayerContext:
    ...
    async def __aenter__(self):
        self.log.debug("Setting up modules")
        self.log.info(f"Log level : {self.config.log_level.upper()}")
        for mod_type, mods in self.modules.items():
            self.log.debug(f"{mod_type} : {list(mods.keys())}")
        for mod_type, mods in self.modules.items():
            for mod_name, mod in mods.items():
                deps = self.composer.get_deps(mod_name, mod.__class__.__mod_requires__)
                await try_await(mod.setup(self.config, *deps))
                self.log.info(f"Module set up : {mod_name}")
        return self

    async def __aexit__(self, exc_type: Type[Exception], exc: Exception, tb):
        if exc is not None:
            self.log.error(f"Closing due to exception {str(exc)}")
            traceback.print_exception(exc)
            # self.log.error()
        self.log.debug("Cleaning up modules")
        for mods in self.modules.values():
            for mod in mods.values():
                await try_await(mod.cleanup())
        self.log.debug("Cleaned up modules")

    async def run(self):
        self.log.debug("Running modules")
        tasks: Dict[str, asyncio.Task] = {}
        daemon_tasks: Dict[str, asyncio.Task] = {}
        for mods in self.modules.values():
            for mod in mods.values():
                (daemon_tasks if mod.__mod_daemon__ else tasks)[mod.__mod_name__] = asyncio.create_task(mod.run(), name=f"TASK_MOD_{mod.__mod_name__}_RUN")

        await asyncio.gather(*tasks.values())
        
        for task in daemon_tasks.values():
            task.cancel()

        self.log.debug(f"{len(tasks)} modules finished, {len(daemon_tasks)} modules terminated")
...
class PlayerComposer:
    ...
    def get_deps(self, req_mod_name: str, reqs: list[str | Type[ModuleInterface]]):
        deps = []
        for req in reqs:
            dep = {}
            ...
            if len(dep) == 0:
                print(reqs)
                raise Exception(f"{req_mod_name}: Module dependency not found : {req}")
            else:
    ...
    def create_arg_parser(self):
        ...
        class ModuleChoices(list[str]):
            ...
            def __contains__(self, option: str):
                return option.lower() == "none" or super().__contains__(option.split(":", 1)[0].lower())

        parser.add_argument("--config", help="Configure using yaml/json", required=False)
        parser.add_argument("-i", "--input", help="Manifest (MPD) file location", type=str, required=False)
        parser.add_argument("-l", "--log_level", help="Log level", choices=["debug", "info"], default="info", type=str)
        ...
        parser.add_argument("--plots_dir", "-p", help="Run directory", required=False)
        ...
        for mod_type, mods in self.module_options.items():
            ...
            parser.add_argument(
                ...
                choices=ModuleChoices(self.module_options[mod_type].keys()),
    ...
    def make_player(self, config: PlayerConfig):
        ...
        for attr_name, val in config.__dict__.items():
            ...
            if not attr_name.startswith("mod_") or val is None:
                continue
    ...
    def register_core_modules(self):
        self.register_module("mpd", [MPDProviderImpl, HLSProviderImpl], single_initializer, "HLS Provider", False, "mpd")
        self.register_module(
            "downloader", [LocalClient, TCPClientImpl, QuicClientImpl], downloader_initializer, "Downloader", False, "local"
        )
        self.register_module("bw", [BandwidthMeterImpl, BandwidthMeterCont], single_initializer, "Bandwidth Estimation", False, "bw_meter")
        self.register_module(
            "abr",
            [
                DashABRController,
        ...
        self.register_module(
            "scheduler", [SchedulerImpl, Scheduler360, MP4SchedulerImpl], single_initializer, "Segment download scheduler", False, "scheduler"
        )
...
def single_initializer(mod_type, mod_name, composer: PlayerComposer) -> Dict[str, Module]:
    ...
    print(mod_type, get_mod_name(mod_name), get_mod_props(mod_name))
