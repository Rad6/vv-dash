...
class QuicClientImpl(Module, DownloadManager):
    ...
    async def setup(self, config: PlayerConfig) -> None:
        ...
        self.quic_configuration = QuicConfiguration(
            alpn_protocols=H3_ALPN, is_client=True, verify_mode=ssl.CERT_NONE,
    ...
    def save_session_ticket(self, ticket: SessionTicket) -> None:
        ...
        is received.
        """
        self.log.info("New session ticket received from server: " + str(ticket.server_name))
    ...
    @critical_task()
    async def start_session(self, host, port, client_up_event=None):
        """
        ...
        async with connect(
            ...
            if client_up_event is not None:
                client_up_event.set()
            self.log.info("Started QUIC session")
    ...
    async def download(self, request: DownloadRequest, save=False) -> Optional[bytes]:
        ...
        if self._client is None:
            ...
            if parsed.port is not None:
                ...
                port = 443
            event = asyncio.Event()
            asyncio.create_task(self.start_session(host, port, client_up_event=event))
            await event.wait()
