...
class HttpProtocol(QuicConnectionProtocol):
    ...
    def quic_event_received(self, event: QuicEvent) -> None:
        # self.log.info(f"Quic Event received : {event}")
    ...
    async def _request(self, request: HttpRequest) -> AsyncIterator[H3Event]:
        ...
        # self.log.info(f"Data and headers sent to {self._http}")
        ...
        while True:
            ...
            try:
                # self.log.info("Waiting for HTTP3 event")
                event = await task
                # self.log.info("HTTP3 event received")
                yield event
