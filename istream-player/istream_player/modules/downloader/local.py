...
class LocalClient(Module, DownloadManager):
    log = logging.getLogger("LocalClient")
    ...
    async def wait_complete(self, url: str) -> Tuple[bytes, int]:
        ...
        """
        self.log.debug(f"Waiting for: {url}")
    ...
    def cancel_read_url(self, url: str):
        ...
        self.log.info(f"Cancelling URL: {url}")
    ...
    async def download(self, request: DownloadRequest, save: bool = False) -> Optional[bytes]:
        self.log.debug(f"Fetching from local FS: {request.url}")
        url = request.url
        ...
        self.read_tasks[url] = asyncio.create_task(self.request_read(url), name=f"TASK_LOCAL_REQREAD_{url.rsplit('/', 1)[-1]}")
        if save:
            await self.transfer_compl[url].wait()
            content = self.content[url]
            return content
