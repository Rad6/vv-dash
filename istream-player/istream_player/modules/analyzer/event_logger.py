...
from istream_player.core import mpd_provider
...
from istream_player.modules.analyzer.exp_events import ExpEvent_PlaybackStart, ExpEvent_Progress, ExpEvent_State
...
class EventLogger(Module, Analyzer, SchedulerEventListener, PlayerEventListener):
    ...
    async def run(self):
        self.recorder.write_event(ExpEvent_PlaybackStart(round(time.time() * 1000)))
    ...
    def total_duration(self):
        ...
        if self._total_duration is None:
            self._total_duration = max(
                sum(seg.duration for seg in rep.segments.values())
                for adap_set in self.mpd_provider.mpd.adaptation_sets.values() 
    ...
    async def on_position_change(self, position):
        ...
        self.log.info(f"JOB_PROGRESS = {progress}")
